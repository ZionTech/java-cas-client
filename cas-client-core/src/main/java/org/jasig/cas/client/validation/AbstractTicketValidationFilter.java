 ...
 * following location: http://www.apache.org/licenses/LICENSE-2.0 Unless
    ...
    extends AbstractCasFilter
{
  ...
  protected Properties getSSLConfig()
  {
    ...
    final String fileName =
        getString(ConfigurationKeys.SSL_CONFIG_FILE);
    if (fileName != null)
    {
  ...
  {
    final Class<? extends HostnameVerifier> className =
        getClass(ConfigurationKeys.HOSTNAME_VERIFIER);
    final String config =
        getString(ConfigurationKeys.HOSTNAME_VERIFIER_CONFIG);
    if (className != null)
    {
      if (config != null)
      {
  ...
  {
    setExceptionOnValidationFailure(getBoolean(
        ConfigurationKeys.EXCEPTION_ON_VALIDATION_FAILURE));
    setRedirectAfterValidation(
        getBoolean(ConfigurationKeys.REDIRECT_AFTER_VALIDATION));
   ...
   */
  protected void onSuccessfulValidation(
    ...
    if (CommonUtils.isNotBlank(ticket))
    {
      ...
      try
      {
        ...
        if (this.redirectAfterValidation)
        {
          ...
          response
              .sendRedirect(constructServiceUrl(request, response));
          return;
        }
      }
      catch (final TicketValidationException | IOException
          | RuntimeException e)
      {
  ...
  private String getPlanEndPointServerName(final String tenant)
  {
    ...
    final DeploymentPlan deploymentPlan =
        provider.getDeploymentPlan();
    CommonUtils.assertNotNull(deploymentPlan,
        "Deployment plan can't be null");
    final String endPoint = deploymentPlan
        .getServiceRestEndPoint(ServiceType.cas, tenant);
      ...
      throws IOException, ServletException
  {
    ...
    if (!CommonUtils.isBlank(authHeader) && authHeader.toLowerCase()
        .startsWith("Bearer".toLowerCase() + ' '))
    {
      final String accessToken =
          authHeader.substring("Bearer".length() + 1);
      ...
      try
      {
        ...
        {
          serverName =
              getPlanEndPointServerName(serverName.substring(0, pos));
        }
        CommonUtils.assertNotNull(serverName,
            "Server name can't be null");
        ...
        final String serverResponse = CommonUtils
            .getResponseFromServer(new URL(builder.toString()),
                new HttpsURLConnectionFactory(),
                getString(ConfigurationKeys.ENCODING));
        logger.debug("CAS Profile Server response: {}",
            serverResponse);
        final JsonParser parser = new JsonParser();
        final JsonReader reader =
            new JsonReader(new StringReader(serverResponse.trim()));
        reader.setLenient(true);
        final JsonObject responseFromServer =
            (JsonObject) parser.parse(reader);
        Iterator<Entry<String, JsonElement>> itr =
            responseFromServer.entrySet().iterator();
        ...
        while (itr.hasNext())
        {
          ...
          {
            final JsonArray attributesArray =
                entry.getValue().getAsJsonArray();
            final Iterator<JsonElement> attrItr =
                attributesArray.iterator();
            while (attrItr.hasNext())
            {
              final JsonObject jo = (JsonObject) attrItr.next();
              final Iterator<Entry<String, JsonElement>> joItr =
                  jo.entrySet().iterator();
              while (joItr.hasNext())
              {
                final Entry<String, JsonElement> attr = joItr.next();
                if (attr.getValue() instanceof JsonArray)
                {
                  // TODO convert json array to string array
                  final JsonArray arr =
                      attr.getValue().getAsJsonArray();
                  final Iterator<JsonElement> attrElementItr =
                      arr.iterator();
                  final List<String> strArray =
        ...
        CommonUtils.assertNotNull(tgtId,
            "Ticket granting ticket can't be null");
        if (!tgtId.trim().isEmpty())
        {
          logger.debug("TGT ID {}", tgtId);
          String requestUri =
              String.format("/auth/v1/tickets/%s", tgtId);
          builder = new StringBuilder();
          builder.append(serverNamePort).append(requestUri);
          final Map<String, String> postParams =
              new HashMap<String, String>();
          postParams.put("service",
              request.getRequestURL().toString());
          logger.debug("TGT request URL {}", requestUri);
          final String serviceTicket = CommonUtils
              .getResponseFromServer(new URL(builder.toString()),
                  new HttpsURLConnectionFactory(),
                  getString(ConfigurationKeys.ENCODING), null,
                  postParams);
          CommonUtils.assertNotNull(serviceTicket,
              "Service ticket can't be null");
          logger.debug("Service ticket {}", serviceTicket);
          builder = new StringBuilder();
          requestUri = String.format(
              "/auth/p3/proxyValidate?ticket=%s&service=%s&pgtUrl=%s",
              serviceTicket,
              request.getRequestURL(),
              getString(ConfigurationKeys.PROXY_CALLBACK_URL));
          builder.append(serverNamePort).append(requestUri);
          // Create a map for header information and put an extra command to
          // remove the service ticket after PT is created.
          final Map<String, String> headers =
              new HashMap<String, String>();
          headers.put("x-command", "rm-st");
          logger.debug("Proxy validate URL {}", builder.toString());
          final String validateResponse = CommonUtils
              .getResponseFromServer(new URL(builder.toString()),
                  new HttpsURLConnectionFactory(),
                  getString(ConfigurationKeys.ENCODING), headers);
          logger.debug("Proxy validate response {}",
              validateResponse);
          final String proxyGrantingTicketIou =
              XmlUtils.getTextForElement(validateResponse,
                  "proxyGrantingTicket");
          logger.debug("Proxy Granting Ticket IOU {}",
              proxyGrantingTicketIou);
          final String proxyGrantingTicket;
          // Cast this class to use the method implemented in the parent class.
          final Cas30ProxyTicketValidator validator =
              (Cas30ProxyTicketValidator) this.ticketValidator;
          ...
          {
            proxyGrantingTicket =
                validator.getProxyGrantingTicketStorage()
                    .retrieve(proxyGrantingTicketIou);
          }
          CommonUtils.assertNotNull(proxyGrantingTicket,
              "Proxy Granting Ticket cannot be null");
          logger.debug("Proxy Granting Ticket {}",
              proxyGrantingTicket);
          ...
          assertion =
              new AssertionImpl(new AttributePrincipalImpl(principal,
                  attributes, proxyGrantingTicket,
  ...
  public final void setTicketValidator(
